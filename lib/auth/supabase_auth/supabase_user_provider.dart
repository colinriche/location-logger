...
class AParkingCompanionSupabaseUser extends BaseAuthUser {
  AParkingCompanionSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> aParkingCompanionSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AParkingCompanionSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
